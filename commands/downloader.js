        ...
        async(Void, citel, text) => {
            ...
            const getRandom = (ext) => { return `${Math.floor(Math.random() * 10000)}${ext}`;  };
                ...
                let stats = fs.statSync(`./${randomName}`);
                 await Void.sendMessage(citel.chat, {  video: fs.readFileSync(`./${randomName}`),jpegThumbnail: log0, mimetype: 'video/mp4',  caption: ` ───⦁⇆ㅤ ||◁ㅤ❚❚ㅤ▷||ㅤ ↻⦁───\n\n❒ Title : ${titleYt}\n\n𝗚𝗘𝗡𝗘𝗥𝗔𝗧𝗘𝗗 𝗕𝗬 𝗩𝗔𝗝𝗜𝗥𝗔`, }, { quoted: citel })
...
cmd({
            pattern: "apk",
            alias: ["downapk"],
            desc: "Downloads apps from playstore.",
            category: "downloader",
	    react: "⚡",
            filename: __filename,
            use: '<apk app name>',
...
cmd({
        ...
        async(Void, citel, text) => {
            let yts = require("secktor-pack");
            if (!text) return citel.reply(`Example : ${prefix}yts WhatsApp Bot by •ᴠᴀᴊɪʀᴀ-ᴍᴜʟᴛɪᴅᴇᴠɪᴄᴇ•`);
            let search = await yts(text);
